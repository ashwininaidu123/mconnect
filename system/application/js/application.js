...
jQuery(document).ready(function () {
	...
	$('#tid').live('change',function(event){
		var tid=$('#tid').val();
		$('#content').val('');
		$.get("Email/TemplateC/"+tid, function(data){
			var data = $(data).text();
    ...
    //~ $('.clickToConnect').live('click',function(event){
		...
		//~ return false;	
	//~ });
	  $('.mcubecalls').live('click',function(event){
		event.preventDefault();
		var links=this.href;
		$.alerts.okButton = 'Yes';
		$.alerts.cancelButton = 'No';
	...
	$('.blkDeletecall').live('click',function(event){
		...
		/*=====blk Del site====*/
	$('.blkDelsite').live('click',function(event){
		var a = new Array();
		var nurl=$('.blkDelsite').attr('href');
		$(".blk_check").each(function(){
			if($(this).is(":checked")){
				a.push($(this).val());
			}
...
$(document).ready(function() {
    ...
    $(add_button).click(function(e){ //on add input button click
        ...
        if(x < max_fields){ //max input box allowed
            ...
            $(wrapper).append('<div><input class="form-control" type="text" name="intrestopt[]" placeholder ="Your Option"><a href="#" class="remove_field">Remove</a></div>'); //add input box
        }
    });
...
$(document).ready(function() {
    ...
    var wrapper         = $(".input_img_wrap"); //Fields wrapper
    var add_button_loc      = $(".add_img_button"); //Add button ID
    ...
    $(add_button_loc).click(function(e){ //on add input button click
        ...
        if(x < max_fields){ //max input box allowed
            x++; //text box increment
            $(wrapper).append('<div><input id="loc_image"  type="file" name="locimage[]"/><a href="#" class="remove_field">Remove</a></div>'); //add input box
        }
    });
...
$(document).ready(function() {
    var max_fields      = 3; //maximum input boxes allowed
    var wrapper         = $(".input_image_wrap"); //Fields wrapper
    ...
    $(add_button).click(function(e){ //on add input button click
        ...
        if(x < max_fields){ //max input box allowed
            x++; //text box increment
            $(wrapper).append('<div><input id="siteimage"  type="file" name="siteimage[]"/><a href="#" class="remove_field">Remove</a></div>'); //add input box
        }
    });
...
var dis1 = document.getElementById("sitevideo");
dis1.onchange = function () {
   if (this.value != "" || this.value.length > 0) {
      document.getElementById("siteimg").disabled = true;
      document.getElementById("add_image_button").style.display = 'none';
   }else{
	    document.getElementById("siteimg").disabled = false;
   }
}
var dis2 = document.getElementById("siteimg");
dis2.onchange = function () {
   if (this.value != "" || this.value.length > 0) {
      document.getElementById("sitevideo").disabled = true;
   }else{
	    document.getElementById("sitevideo").disabled = false;
   }
}
