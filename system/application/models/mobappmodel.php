...
class Mobappmodel extends Model
{
    ...
    function mob_auth($data)
    {
        ...
        if ($rest->num_rows() > 0) {
            ...
            $sql = "SELECT otp FROM mconnect_numotp
    ...
    function authenticate()
    {
        ...
        if ($rst->num_rows() > 0) {
            ...
            if ($retArr['otp'] == $otp) {
                $sql = "DELETE FROM mconnect_numotp WHERE otp ='" . $otp . "'";
    ...
    function register()
    {
        ...
        if ($rest->num_rows() > 0) {
            ...
            $sql     = "INSERT INTO mconnect_register SET  uid	='" . $id . "',usrname ='" . $_POST['name'] . "', usremail ='" . $_POST['email'] . "',usrnumber ='" . $_POST['number'] . "',password =md5('" . $_POST['password'] . "'), authkey ='" . $authkey . "'";
            $ret     = $this->db->query($sql);
            return $ret;
        }
    }
    ...
    {
        $retArr = array();
        $sql    = "SELECT otp FROM mconnect_register WHERE otp='" . $_POST['otp'] . "'";
        $rest   = $this->db->query($sql);
        if ($rest->num_rows() > 0) {
    ...
    function checkvisit()
    {
        ...
        $sql      = "SELECT `business`,`siteid` FROM beacon 
        ...
        $sql  = "SELECT * FROM visited_history WHERE authKey = '" . $authKey . "' AND siteid = '" . $siteid . "'";
        $rest = $this->db->query($sql);
        if ($rest->num_rows() > 0) {
            // IF location is visited or Not
            $sql  = "SELECT * FROM visited_history WHERE authKey = '" . $authKey . "' AND siteid = '" . $siteid . "' AND beaconid = '" . $beaconid . "'";
            $rest = $this->db->query($sql);
            if ($rest->num_rows() > 0) {
                $sql  = "SELECT * FROM visited_history WHERE authKey = '" . $authKey . "' AND siteid = '" . $siteid . "' AND beaconid = '" . $beaconid . "' AND detail_sent = '0' ";
                $rest = $this->db->query($sql);
                if ($rest->num_rows() > 0) {
                    $sql  = "SELECT o.beaconid,s.sitename AS 'name',s.bid,s.siteid,s.siteicon as 'logo',n.landingnumber as 'number', o.locname AS 'lname',o.loc_desc AS 'desc',o.loc_image as 'media',ul.liked as 'like' FROM " . $bid . "_mc_location o
    ...
    function likesite()
    {
        ...
        $siteid = $_POST['siteid'];
        $bid = $_POST['bid'];
        $sql    = "SELECT * FROM user_likes WHERE siteid ='" . $siteid . "' AND authKey ='" . $authKey . "' AND bid ='" . $bid . "'";
        $rest   = $this->db->query($sql);
        if ($rest->num_rows() > 0) {
            $retArr = $rest->row_array();
        if($retArr['liked'] == '1'){
        $sql     = "DELETE FROM user_likes WHERE authKey ='" . $authKey . "' AND siteid ='" . $siteid . "' AND bid ='" . $bid . "'";
        $rest    = $this->db->query($sql);
	    return false;
	    }else{
	    $this->db->set('liked',1);
     ...
     function getallsites()
    {
		...
		foreach ($rst as $key => $value) {
         $bid = $rst[$key]['bid'];
         $sql     = "SELECT v.sitename,v.time,v.bid,v.siteid,v.siteicon,v.sitedesc,ul.authKey,ul.liked as `like`,a.offerper,a.offer_desc,n.landingnumber as 'number' FROM  ".$bid."_site v
	     LEFT JOIN  prinumbers n ON n.pri = v.tracknum
	     LEFT JOIN  offers a ON a.siteid = v.siteid
	     LEFT JOIN  user_likes ul ON ul.siteid = v.siteid AND ul.authkey ='". $authkey."'
    ...
    { 
        ...
        $sql     = "SELECT * FROM referrals r
	         WHERE r.authKey ='". $authkey."' AND r.siteid ='". $siteid."' AND r.referedtonum ='". $number."'";
                  $rest   = $this->db->query($sql);
		 if ($rest->num_rows() > 0) {
                   return false;
         }else{
              $sql = "INSERT INTO referrals SET  authKey	='" . $authkey . "', siteid ='" . $siteid . "',referedto ='" .$_POST['name'] . "', referedtonum	 ='" . $number . "', referedtoemail	 ='" . $_POST['email'] . "', comment	 ='" . $_POST['message'] . "'";
            $ret = $this->db->query($sql);
        return true;
	}
    }
