...
//		echo $this->db->last_query();
	...
	function trackd(){
		$res=array();
		$sql=$this->db->query("SELECT COALESCE(COUNT(callid), 0) AS day 
							FROM ".$this->session->userdata('bid')."_callhistory h 
							WHERE (h.`starttime`  > DATE_SUB(NOW(), INTERVAL 1 DAY))");				
		if($sql->num_rows()>0){
			$res = $sql->row();
			return $res->day;
		}else{
	...
	function trackw(){
		$res=array();
		$sql=$this->db->query("SELECT COALESCE(COUNT(callid), 0) AS week 
							FROM ".$this->session->userdata('bid')."_callhistory h 
	...
	function trackm(){
		$res=array();
		$sql=$this->db->query("SELECT COALESCE(COUNT(callid), 0) AS month 
							FROM ".$this->session->userdata('bid')."_callhistory h 
							WHERE (h.`starttime`  > DATE_SUB(NOW(), INTERVAL 1 MONTH))");				
		if($sql->num_rows()>0){
			$res = $sql->row();
			return $res->month;
		}else{
		...
		function mcubexd(){
		$res=array();
		$sql=$this->db->query("SELECT COALESCE(COUNT(pbxid), 0) AS day 
							FROM ".$this->session->userdata('bid')."_pbxreport h 
							WHERE (h.`starttime`  > DATE_SUB(NOW(), INTERVAL 1 DAY))");				
		if($sql->num_rows()>0){
			$res = $sql->row();
			return $res->day;
	...
	function mcubexw(){
		$res=array();
		$sql=$this->db->query("SELECT COALESCE(COUNT(pbxid), 0) AS week 
							FROM ".$this->session->userdata('bid')."_pbxreport h 
							WHERE (h.`starttime`  > DATE_SUB(NOW(), INTERVAL 1 WEEK))");				
		if($sql->num_rows()>0){
			$res = $sql->row();
	...
	function mcubexm(){
		$res=array();
		$sql=$this->db->query("SELECT COALESCE(COUNT(pbxid), 0) AS month 
							FROM ".$this->session->userdata('bid')."_pbxreport h 
							WHERE (h.`starttime`  > DATE_SUB(NOW(), INTERVAL 1 MONTH))");				
		if($sql->num_rows()>0){
			$res = $sql->row();
	...
	function ivrsd(){
		$res=array();
		$sql=$this->db->query("SELECT COALESCE(COUNT(hid), 0) AS day 
							FROM ".$this->session->userdata('bid')."_ivrshistory h 
							WHERE (h.`datetime`  > DATE_SUB(NOW(), INTERVAL 1 DAY))");				
		if($sql->num_rows()>0){
			$res = $sql->row();
			return $res->day;
		}else{
	...
	function ivrsw(){
		$res=array();
		$sql=$this->db->query("SELECT COALESCE(COUNT(hid), 0) AS week 
							FROM ".$this->session->userdata('bid')."_ivrshistory h 
	...
	function ivrsm(){
		$res=array();
		$sql=$this->db->query("SELECT COALESCE(COUNT(hid), 0) AS month 
							FROM ".$this->session->userdata('bid')."_ivrshistory h 
							WHERE (h.`datetime`  > DATE_SUB(NOW(), INTERVAL 1 MONTH))");				
		if($sql->num_rows()>0){
			$res = $sql->row();
			return $res->month;
		}else{
	...
	function leadd(){
		$res=array();
		$sql=$this->db->query("SELECT COALESCE(COUNT(leadid), 0) AS day 
							FROM ".$this->session->userdata('bid')."_leads h 
							WHERE (h.`createdon`  > DATE_SUB(NOW(), INTERVAL 1 DAY))");				
		if($sql->num_rows()>0){
			$res = $sql->row();
			return $res->day;
		}else{
	...
	function leadw(){
		$res=array();
		$sql=$this->db->query("SELECT COALESCE(COUNT(leadid), 0) AS week 
							FROM ".$this->session->userdata('bid')."_leads h 
							WHERE (h.`createdon`  > DATE_SUB(NOW(), INTERVAL 1 WEEK))");				
		if($sql->num_rows()>0){
			$res = $sql->row();
			return $res->week;
		}else{
	...
	function leadm(){
		$res=array();
		$sql=$this->db->query("SELECT COALESCE(COUNT(leadid), 0) AS month 
							FROM ".$this->session->userdata('bid')."_leads h 
							WHERE (h.`createdon`  > DATE_SUB(NOW(), INTERVAL 1 MONTH))");				
		if($sql->num_rows()>0){
			$res = $sql->row();
			return $res->month;
		}else{
	...
	function supportd(){
		$res=array();
		$sql=$this->db->query("SELECT COALESCE(COUNT(tktid), 0) AS day 
	...
	function supportw(){
		$res=array();
		$sql=$this->db->query("SELECT COALESCE(COUNT(tktid), 0) AS week 
							FROM ".$this->session->userdata('bid')."_support_tickets h 
							WHERE (h.`createdon`  > DATE_SUB(NOW(), INTERVAL 1 WEEK))");				
		if($sql->num_rows()>0){
			$res = $sql->row();
	...
	function supportm(){
		$res=array();
		$sql=$this->db->query("SELECT COALESCE(COUNT(tktid), 0) AS month 
							FROM ".$this->session->userdata('bid')."_support_tickets h 
							WHERE (h.`createdon`  > DATE_SUB(NOW(), INTERVAL 1 MONTH))");				
		if($sql->num_rows()>0){
			$res = $sql->row();
	...
	function outboundd(){
		$res=array();
		$sql=$this->db->query("SELECT COALESCE(COUNT(callid), 0) AS day 
							FROM ".$this->session->userdata('bid')."_outboundcalls h 
							WHERE (h.`starttime`  > DATE_SUB(NOW(), INTERVAL 1 DAY))");				
		if($sql->num_rows()>0){
			$res = $sql->row();
			return $res->day;
